...
class PayoutHistoryCell : CustomCell{
    ...
    override func setUpCell() {
        ...
        statusIcon.anchor(top: nil, left: self.leadingAnchor, right: nil, bottom: nil, paddingTop: 0, paddingLeft: 10, paddingRight: 0, paddingBottom: 0, width: 40, height: 40)
        statusIcon.centerYAnchor.constraint(equalTo: self.centerYAnchor).isActive = true
        
        
        let bankStack =  StackManager.createStackView(with: [bankLabel, dateLabel], axis: .horizontal, distribution: .fillProportionally, alignment: .center)
